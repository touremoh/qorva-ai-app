					...
					{projectsAndAchievements.map((project, i) => (
						<Grid2 key={i} size={{ xs: 12, sm: 6 }}>
							<Box sx={{ p: 1.5, textAlign: 'left', backgroundColor: tokens.surface.subtle, borderRadius: 1.5, border: `1px solid ${tokens.surface.muted}` }}>
								...
								{project.description && (
									<Typography sx={{ fontSize: '0.78rem', color: tokens.ink.muted, mt: 0.5, lineHeight: 1.6 }}>
										{project.description}
									</Typography>
								)}
								<Box sx={{ display: 'flex', gap: 1, mt: 0.75, flexWrap: 'wrap' }}>
									{project.date && (
										<Typography sx={{ fontSize: '0.72rem', color: tokens.ink.subtle }}>{project.date}</Typography>
									)}
									{project.impact && (
										<Typography sx={{ fontSize: '0.72rem', color: tokens.brand.text, fontWeight: 600 }}>
											{project.impact}
										</Typography>
									)}
								</Box>
