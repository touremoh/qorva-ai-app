...
const ClusteringTabContent = ({ clustering, t }) => {
	...
	return (
		<Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
			...
			<Paper elevation={0} sx={{ p: 2.5, borderRadius: 2, border: `1px solid ${tokens.line.main}` }}>
				<Box sx={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 2, flexWrap: 'wrap' }}>
					<Box sx={{ flex: 1, minWidth: 0 }}>
						<Typography sx={{ fontSize: tokens.fontSize.micro, fontWeight: 700, color: tokens.ink.subtle, textTransform: 'uppercase', letterSpacing: '0.07em', mb: 0.75 }}>
							{t('appCVMatching.clustering.sectionTitle', 'Candidate Clustering')}
						</Typography>
						{cl.primaryCluster && (
							<Chip label={cl.primaryCluster} sx={{
								height: 'auto', py: 0.75, px: 0.5, fontSize: tokens.fontSize.body2, fontWeight: 700,
					...
					{confPct != null && (
						<Box sx={{ minWidth: 100, textAlign: 'right' }}>
							<Typography sx={{ fontSize: tokens.fontSize.micro, fontWeight: 700, color: tokens.ink.subtle, textTransform: 'uppercase', letterSpacing: '0.07em', mb: 0.5 }}>
								{t('appCVMatching.clustering.confidence', 'Cluster Confidence')}
							</Typography>
							<Typography sx={{ fontSize: tokens.fontSize.xxl, fontWeight: 800, color: tokens.brand.text, lineHeight: 1 }}>
								{confPct}%
							</Typography>
			...
			{attrRows.length > 0 && (
				<Paper elevation={0} sx={{ p: 2.5, borderRadius: 2, border: `1px solid ${tokens.line.main}` }}>
					<Typography sx={{ fontSize: tokens.fontSize.micro, fontWeight: 700, color: tokens.ink.subtle, textTransform: 'uppercase', letterSpacing: '0.07em', mb: 1.25 }}>
					...
					<Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap>
						{attrRows.map(row => (
							<Box key={row.key} sx={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: 0.25 }}>
								<Typography sx={{ fontSize: tokens.fontSize.micro, color: tokens.ink.subtle, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
									{row.label}
								</Typography>
								<Chip label={toLabel(row.value)} size="small" sx={{
									height: 22, fontSize: tokens.fontSize.caption, fontWeight: 600,
			...
			{(cl.functionalExpertise?.length > 0 || cl.industryDomains?.length > 0 || cl.environmentFit?.length > 0) && (
				<Grid2 container spacing={2}>
							...
							<Paper elevation={0} sx={{ p: 2, borderRadius: 2, border: `1px solid ${tokens.line.main}`, height: '100%' }}>
								...
								<Stack direction="row" spacing={0.5} flexWrap="wrap" useFlexGap>
									{cl.functionalExpertise.map((fe, i) => (
										<Chip key={i} label={fe} size="small" sx={{ height: 22, fontSize: tokens.fontSize.caption, fontWeight: 500, backgroundColor: alpha(tokens.brand.main, 0.08), color: tokens.status.success.text, border: `1px solid ${alpha(tokens.brand.main, 0.2)}` }} />
									))}
								</Stack>
							</Paper>
						</Grid2>
					...
					{cl.industryDomains?.length > 0 && (
						<Grid2 size={{ xs: 12, sm: 4 }}>
							<Paper elevation={0} sx={{ p: 2, borderRadius: 2, border: `1px solid ${tokens.line.main}`, height: '100%' }}>
								<Typography sx={{ fontSize: tokens.fontSize.micro, fontWeight: 700, color: tokens.ink.subtle, textTransform: 'uppercase', letterSpacing: '0.07em', mb: 1 }}>
									{t('appCVMatching.clustering.industryDomains', 'Industry Domains')}
								</Typography>
								<Stack direction="row" spacing={0.5} flexWrap="wrap" useFlexGap>
									{cl.industryDomains.map((d, i) => (
										<Chip key={i} label={d} size="small" sx={{ height: 22, fontSize: tokens.fontSize.caption, fontWeight: 500, backgroundColor: tokens.surface.muted, color: tokens.ink.body, border: `1px solid ${tokens.line.main}` }} />
									))}
								</Stack>
							</Paper>
						</Grid2>
							...
							<Paper elevation={0} sx={{ p: 2, borderRadius: 2, border: `1px solid ${tokens.line.main}`, height: '100%' }}>
								...
								<Stack direction="row" spacing={0.5} flexWrap="wrap" useFlexGap>
									{cl.environmentFit.map((e, i) => (
										<Chip key={i} label={toLabel(e)} size="small" sx={{ height: 22, fontSize: tokens.fontSize.caption, fontWeight: 500, backgroundColor: tokens.status.info.pale, color: tokens.status.info.main, border: `1px solid ${tokens.status.info.border}` }} />
									))}
								</Stack>
							</Paper>
						</Grid2>
					...
					<Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.75 }}>
						{cl.businessImpact.map((impact, i) => (
							<Box key={i} sx={{ display: 'flex', alignItems: 'flex-start', gap: 0.75 }}>
								<CheckCircleOutlineOutlinedIcon sx={{ fontSize: tokens.iconSize.sm, color: tokens.brand.text, mt: 0.2, flexShrink: 0 }} />
								<Typography sx={{ fontSize: tokens.fontSize.body2, color: tokens.ink.body, lineHeight: 1.55 }}>
									{impact}
								</Typography>
							</Box>
						))}
				...
				<Paper elevation={0} sx={{ p: 2.5, borderRadius: 2, border: `1px solid ${tokens.line.main}` }}>
					...
					<Stack direction="row" spacing={0.5} flexWrap="wrap" useFlexGap>
						{cl.secondaryClusters.map((sc, i) => (
							<Chip key={i} label={sc} size="small" sx={{ height: 22, fontSize: tokens.fontSize.caption, fontWeight: 500, backgroundColor: 'rgba(99,102,241,0.05)', color: tokens.status.accent.bright, border: '1px solid rgba(99,102,241,0.15)' }} />
						))}
					</Stack>
				</Paper>
			)}
			...
			{cl.clusterReasoning && (
				<Paper elevation={0} sx={{ p: 2.5, borderRadius: 2, border: `1px solid ${tokens.line.main}` }}>
					...
					<Typography sx={{ fontSize: tokens.fontSize.body2, color: tokens.ink.muted, lineHeight: 1.65, fontStyle: 'italic' }}>
						{cl.clusterReasoning}
					</Typography>
				</Paper>
			)}
