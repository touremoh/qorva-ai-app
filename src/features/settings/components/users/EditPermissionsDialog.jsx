		...
		<Dialog open={!!editUser} onClose={() => { if (!savingEdit) setEditUser(null); }} maxWidth="md" fullWidth slotProps={{ paper: DIALOG_PAPER_SX }}>
			<DialogTitle sx={{ pb: 1 }}>
				<Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flexWrap: 'wrap' }}>
					<ManageAccountsOutlinedIcon sx={{ fontSize: tokens.iconSize.lg, color: tokens.brand.text }} />
			...
			<DialogActions sx={{ px: 3, py: 2, gap: 1 }}>
				<Button onClick={() => setEditUser(null)} disabled={savingEdit} sx={{ borderRadius: 2, textTransform: 'none', fontSize: tokens.fontSize.body2, color: tokens.ink.muted }}>
