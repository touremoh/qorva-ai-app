...
const ReportCandidateHeader = ({ candidate, jobTitle, nameInitials }) => {
	...
	return (
		...
		<Box sx={{
			...
			<Avatar sx={{ width: 40, height: 40, fontSize: '0.85rem', fontWeight: 700, backgroundColor: THEME_GREEN, color: tokens.ink.inverse }}>
				{nameInitials}
			</Avatar>
			<Box sx={{ flex: 1, minWidth: 0 }}>
				<Typography sx={{ fontWeight: 700, fontSize: '0.95rem', color: tokens.ink.strong, lineHeight: 1.3 }}>
					{candidate.candidateName}
					{candidate.nbYearsExperience != null && (
						<Typography component="span" sx={{ fontSize: '0.78rem', fontWeight: 400, color: tokens.ink.muted, ml: 1 }}>
							• {candidate.nbYearsExperience} {t('appCVContent.yearsAbbr')} {t('appCVContent.experience')}
						</Typography>
					)}
				</Typography>
				<Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, mt: 0.25 }}>
					<WorkOutlineOutlinedIcon sx={{ fontSize: 13, color: tokens.ink.subtle }} />
					<Typography sx={{ fontSize: '0.75rem', color: tokens.ink.muted }}>{jobTitle}</Typography>
				</Box>
			</Box>
		</Box>
		</>
