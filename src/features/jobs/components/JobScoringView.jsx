...
const importanceChipSx = (importance) => {
	if (importance === 'mandatory') return { fontSize: tokens.fontSize.caption, backgroundColor: alpha(tokens.brand.main, 0.10), color: tokens.brand.dark, borderRadius: 0.75, height: 22, fontWeight: 600 };
	if (importance === 'important') return { fontSize: tokens.fontSize.caption, backgroundColor: 'rgba(245,158,11,0.12)', color: tokens.status.warning.main, borderRadius: 0.75, height: 22, fontWeight: 600 };
	return { fontSize: tokens.fontSize.caption, backgroundColor: tokens.surface.muted, color: tokens.ink.soft, borderRadius: 0.75, height: 22, fontWeight: 500 };
};
...
const statLabelSx = { fontSize: tokens.fontSize.caption, color: tokens.ink.subtle, textTransform: 'uppercase', letterSpacing: '0.05em' };

const JobScoringView = ({ scoringRules, t }) => {
	if (!scoringRules) return (
		<Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', p: 4 }}>
			<Typography sx={{ fontSize: tokens.fontSize.body2, color: tokens.ink.subtle }}>{t('jobContent.noMatchingRules')}</Typography>
		</Box>
	);
						...
						{sr.skills.map((s, i) => (
							<Box key={i} sx={i > 0 ? { pt: 1.5, mt: 1.5, borderTop: `1px solid ${tokens.surface.muted}` } : {}}>
								<Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 0.5, mb: 0.75 }}>
									<Typography sx={{ fontWeight: 700, fontSize: tokens.fontSize.body2, color: tokens.ink.strong }}>{s.name}</Typography>
								...
								<Box sx={{ display: 'flex', gap: 0.75, flexWrap: 'wrap' }}>
									<Chip label={`${weightPct(s.weight)} ${t('jobContent.weightLabel')}`} size="small"
										sx={{ fontSize: tokens.fontSize.caption, height: 22, backgroundColor: alpha(tokens.brand.main, 0.10), color: tokens.brand.dark, borderRadius: 0.75 }} />
									<Chip label={`${s.minYearsOfExperience} ${t('jobContent.yearsAbbr')} min.`} size="small"
										sx={{ fontSize: tokens.fontSize.caption, height: 22, backgroundColor: tokens.surface.muted, color: tokens.ink.muted, borderRadius: 0.75 }} />
									{s.exactSkillOnly && (
										<Chip label={t('jobContent.exactSkillOnly')} size="small"
											sx={{ fontSize: tokens.fontSize.caption, height: 22, backgroundColor: tokens.status.info.paleAlt, color: tokens.status.info.blue, borderRadius: 0.75 }} />
									)}
								</Box>
							</Box>
						))}
					...
					<Box sx={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
						{sr.experienceRequirements.minYearsOfExperience != null && (
							<Box>
								...
								<Typography sx={{ fontWeight: 700, fontSize: tokens.fontSize.body2, color: tokens.ink.strong }}>{sr.experienceRequirements.minYearsOfExperience} {t('jobContent.yearsAbbr')}</Typography>
							</Box>
						)}
						{sr.experienceRequirements.minRelevantYears != null && (
							<Box>
								<Typography sx={statLabelSx}>{t('jobContent.minRelevantYears')}</Typography>
								<Typography sx={{ fontWeight: 700, fontSize: tokens.fontSize.body2, color: tokens.ink.strong }}>{sr.experienceRequirements.minRelevantYears} {t('jobContent.yearsAbbr')}</Typography>
							</Box>
						)}
						{sr.experienceRequirements.seniorityLevel && (
							<Box>
								<Typography sx={statLabelSx}>{t('jobContent.seniorityLevel')}</Typography>
								<Typography sx={{ fontWeight: 700, fontSize: tokens.fontSize.body2, color: THEME_GREEN }}>{t(`jobContent.${sr.experienceRequirements.seniorityLevel}`)}</Typography>
							</Box>
						)}
					</Box>
				</CVCard>
						...
						<Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.6, mb: 1.5 }}>
							{sr.locationPreferences.allowedLocations.map(loc => (
								<Chip key={loc} label={loc} size="small"
									sx={{ fontSize: tokens.fontSize.small, backgroundColor: alpha(tokens.brand.main, 0.10), color: tokens.brand.dark, borderRadius: 1, height: 24, fontWeight: 500 }} />
							))}
						</Box>
					)}
					<Box sx={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
						<Box>
							...
							<Typography sx={{ fontWeight: 700, fontSize: tokens.fontSize.body2, color: sr.locationPreferences.remoteAllowed ? THEME_GREEN : `${tokens.ink.strong}` }}>
						...
						{sr.locationPreferences.strictness && (
							<Box>
								...
								<Typography sx={{ fontWeight: 700, fontSize: tokens.fontSize.body2, color: tokens.ink.strong }}>{t(`jobContent.${sr.locationPreferences.strictness}`)}</Typography>
							</Box>
						)}
					</Box>
				</CVCard>
			...
			{sr.industryPreferences && (
				<CVCard>
						...
						<Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.6, mb: 1.5 }}>
							{sr.industryPreferences.preferredIndustries.map(ind => (
								<Chip key={ind} label={ind} size="small"
									sx={{ fontSize: tokens.fontSize.small, backgroundColor: tokens.surface.muted, color: tokens.ink.soft, borderRadius: 1, height: 24 }} />
							))}
						</Box>
					)}
					{sr.industryPreferences.strictness && (
						<Box>
							<Typography sx={statLabelSx}>{t('jobContent.strictness')}</Typography>
							<Typography sx={{ fontWeight: 700, fontSize: tokens.fontSize.body2, color: tokens.ink.strong }}>{t(`jobContent.${sr.industryPreferences.strictness}`)}</Typography>
						</Box>
					)}
				</CVCard>
			)}
					...
					<Box sx={{ display: 'flex', gap: 1.5, flexWrap: 'wrap' }}>
						{[
							...
							<Box key={key} sx={{ flex: '1 1 80px', p: 1.5, backgroundColor: tokens.surface.subtle, borderRadius: 1.5, border: `1px solid ${tokens.surface.muted}`, textAlign: 'center' }}>
								<Typography sx={{ fontSize: tokens.fontSize.xl, fontWeight: 700, color: THEME_GREEN, lineHeight: 1.2 }}>
							...
							<Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.6 }}>
								{sr.availabilityStatuses.map(status => (
									<Chip key={status} label={t(`jobContent.availabilityStatus.${status}`)} size="small"
										sx={{ fontSize: tokens.fontSize.caption, height: 22, backgroundColor: tokens.status.error.tint, color: tokens.status.error.text, borderRadius: 0.75 }} />
								))}
							</Box>
						</Box>
					)}
